...
def processing_json():
    ...
    raw = request.data.decode('utf-8').strip()   # one request json blob
    with open("%s/%s/Raw.txt"%(path,prefix), "a") as f1:
        f1.write(raw+'\n')
        print('Successfully write the request json blob in Raw.txt!')

    # process json & write to txt
    name_age, flag = extract_json(raw)
    if flag == 0: 
        with open("%s/%s/proc.txt"%(path,prefix), "a") as f2:
            f2.write(name_age+'\n')   # make sure each record in a line
        print('Successfully write the processed info in proc.txt')
